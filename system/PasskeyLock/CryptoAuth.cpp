...
static void sha256_final(SHA256_CTX *ctx, uint8_t *hash) {
    ...
    ctx->data[i++]=(ctx->datalen<56)?0x80:0x80;
    if (ctx->datalen>=56) { while(i<64)ctx->data[i++]=0; sha256_transform(ctx,ctx->data); i=0; }
    while(i<56) ctx->data[i++]=0;
    ctx->bitlen+=ctx->datalen*8;
    for(int k=7;k>=0;k--,ctx->bitlen>>=8) ctx->data[63-(7-k)]=(uint8_t)ctx->bitlen; // big-endian length
    // rewrite last 8 bytes properly
    uint64_t bl = ctx->datalen*8 + (ctx->bitlen - ctx->datalen*8); // already accumulated
    // simplified: just store accumulated bitlen
...
static void hmac_sha256(const uint8_t *key, uint8_t klen,
...
bool hmac_verify(const uint8_t *nonce,    uint8_t nonce_len,
    ...
    if (response_len != 32) return false;
    uint8_t expected[32];
